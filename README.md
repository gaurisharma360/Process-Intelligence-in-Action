# Contribution
Zero cart abandonments were achieved after the suggestions were implemented with no additional cost.[View the report](https://github.com/gaurisharma360/Process-Intelligence-in-Action/blob/main/Final%20report%20PIA_Group4.pdf)
...
### `intro_xes_tutorial_analysis.ipynb`
...
### `intro_xes.ipynb`
The notebook investigates how queues affect cart abandonment: abandonment rate against queue depth over time, queue-selection dynamics (whether customers choose a counter by its line length or by the perceived number of items in the queue), statistically tested if there is an existing counter preferences, and customer waiting time from Enter Queue to first Scan Item. It then examines cart abandonment, whether abandonment cascades over a day, whether higher customer arrivals or more frequent price checks coincide with more abandonment, and the basket sizes of abandoned versus completed carts. The remainder integrates the clerk, counter, and cashier logs to connect customer behaviour with the supporting operations: the number of active clerks per hour, clerk shift durations and schedules, per-clerk price-check and cleanup workloads and efficiency, and price-check durations. Additionally it analyzes per-cashier scanning speed and cashier shift durations alongside active-cashier and active-counter counts per hour, including a comparison of price-check volume against the number of active counters.
